...
typedef struct
{
    int size; //default value;
    int *intP;  //array Pointer
    int currentAmount;
 ...
 *      != 0, ansonsten
 */
extern int stackInit(intstack_t* self); //DONE
 ...
 * @param i     der Wert
 */
extern void stackPush(intstack_t* self, int i); //DONE

/**@brief Gibt das oberste Element des Stacks zurÃ¼ck.
 * @param self  der intstack
 * @return das oberste Element von \p self
 */
extern int stackTop(const intstack_t* self); //DONE

/**@brief Entfernt und liefert das oberste Element des Stacks.
 * @param self  der intstack
 * @return das oberste Element von \p self
 */
extern int stackPop(intstack_t* self); //DONE

/**@brief Gibt zurÃ¼ck, ob der intstack leer ist.
 * @param self  der intstack
